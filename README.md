...
- Python 3.11+
- `pip` (or `uv`, optional)
- Slack app credentials (required now)
- Optional provider keys: OpenAI / OpenRouter / Gemini / Grok

### 1️⃣ Clone the repository

```bash
git clone https://github.com/dipanshuchoudhary-data/Maantra-1.0.git
cd Maantra-1.0
```
...
OPENAI_API_KEY=sk-...            # or ANTHROPIC/OPENROUTER/GEMINI/GROK
...
Slack AI Assistant v2 is ready
```

> ⚠️ If startup fails with provider error, set at least one supported LLM key.
...
- [ ] Slack workspace admin/dev permissions
- [ ] Slack app created at <https://api.slack.com/apps>
- [ ] Bot token + app token generated
- [ ] Socket Mode enabled
...
| `reactions:read` | Reaction events (optional features) |

### 3️⃣ Enable Socket Mode

- Turn on Socket Mode in app settings
- Generate **App-Level Token** with `connections:write`

### 4️⃣ Subscribe to Bot Events
...
| `reaction_added` | Reaction workflows |

### 5️⃣ Configure credentials securely
...
> 🔐 Never commit real tokens. Use `.env` and secret managers in production.

### 6️⃣ Install app to workspace

- Install the app
- Invite bot to channels where needed
...
- Send `help`
- Send `llm show`
- Send `summarize` inside a thread

Expected behavior: assistant responds and logs STEP pipeline in terminal.

---

## Telegram (Roadmap-Ready)

> ℹ️ **Current status**: adapter scaffold is architecture-ready; full Telegram
> adapter implementation is planned in roadmap phase.

### Prerequisites

- Telegram account + BotFather access
- Bot token from BotFather
- Public webhook URL (or polling mode)

### Planned Setup Flow

1. Create bot via BotFather (`/newbot`)
2. Store token securely (`TELEGRAM_BOT_TOKEN`)
3. Configure webhook endpoint or polling worker
4. Map Telegram events to `PlatformMessage`
5. Register adapter with `ChannelManager`
6. Validate normalization + response formatting

### Recommended Permissions/Config

| Item | Recommendation |
|---|---|
| Privacy mode | Disable for group bot workflows if needed |
| Commands | `/start`, `/help`, `/link`, `/memory` |
| Webhook security | Use secret path + TLS |

### Verification (when adapter lands)

- Send `/start`
- Ask context-dependent question
- Validate shared memory continuity with Slack identity link

---

## WhatsApp (Roadmap-Ready)

> ℹ️ **Current status**: planned for final multi-channel rollout phase.
...
1. Create WhatsApp Business app in Meta dashboard
2. Generate long-lived access token
3. Configure webhook URL + verify token
4. Register templates for outbound policy-compliant messages
5. Implement adapter normalization to/from core message schema
6. Link identity with unified account model

### Security Best Practices

| Concern | Best Practice |
|---|---|
| Access token | Store in secrets manager |
| Webhooks | Validate signatures |
| Message windows | Respect 24-hour messaging policy |
| Templates | Use approved templates for outbound workflows |

### Verification (when adapter lands)

- Send inbound test message
- Trigger tool action with shared context
- Confirm response delivery and session persistence
